...
class Settings:
    ...
    threshold: float = float(
        os.getenv("NER_THRESHOLD", "0.3")
    )

    max_text_len: int = int(
        os.getenv("MAX_TEXT_LEN", "4000")
    )
...
class ModelManager:
    ...
    def _resolve_adapter(path: str) -> str:
        ...
        # Local adapter bundled inside the Docker image.
        if p.is_dir():
            return str(p)
    ...
    def extract(
        ...
        if raw is not None:
            ...
            # ({'entities': {label: [texts]}}) + localisation
